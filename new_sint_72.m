...
n_B=8;  % Usi questo o le due righe sotto casuali
...
% Al di là delle T, controllare i risultati sui ruoli:

% C è matrice 4x4
% D = pdist(X) è la distanza euclidea
    ...
    for q=1:4
        if p==1
            if q==1
                for i=1:n_CS
                    for j=1:n_CS
                        CV(p,q)=CV(p,q)+DVs(i,j);
                        CG(p,q)=CG(p,q)+DGs(i,j);
                    end
                end
            elseif q==2
                for i=n_CS+1:n_CS+n_ES_tot
            ...
            elseif q==3
                for i=1+n_CS+n_ES_tot:n_CS+n_ES_tot+n_B
                    for j=1:n_CS
                        CV(p,q)=CV(p,q)+DVs(i,j);
                        CG(p,q)=CG(p,q)+DGs(i,j);
                    end
        ...
        elseif p==3
            if q==1
                for i=1:n_CS
                    for j=1+n_CS+n_ES_tot:n_CS+n_ES_tot+n_B
                        CV(p,q)=CV(p,q)+DVs(i,j);
                        CG(p,q)=CG(p,q)+DGs(i,j);
                    end
                end
